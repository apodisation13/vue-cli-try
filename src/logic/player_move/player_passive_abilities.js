    ...
    if (i === passive_hand.length) {
      deck
        ...
        .forEach(card => {
          ...
          if (cpa === "if-in-deck-increase-self-damage-by-1") {
      ...
      grave
        ...
        .forEach(card => {
          ...
          const cpa = card.passive_ability.name
          if (cpa === "if-in-grave-increase-self-damage-by-1") {
      ...
      const pa = passive_hand[i].passive_ability.name
      if (pa === "damage-random-enemy-by-1") {
        passive_end_turn_damage_random_enemy(passive_hand[i], gameObj)
      } else if (pa === "increase-damage-to-card-in-hand-by-1") {
        passive_end_turn_increase_damage_in_hand(hand)
      } else if (pa === "heal-leader-by-2") {
        passive_end_turn_heal_leader_by_2()
      } else if (pa === "heal-leader-by-3") {
        passive_end_turn_heal_leader_by_3()
      } else if (pa === "increase-self-damage-by-1") {
        passive_end_turn_increase_self_damage(passive_hand[i])
      } else if (pa === "destroy-2-enemies-after-3-turns") {
